...
OTM_points = 100
exp_period = 'currMonth_exp_dt'

straddle_SL_pct = 0.2  #0.2 -> 20%
straddle_tgt_pct = 0.8  #0.8 --> 80%
...
datetime_str = f'{date} {exit_time}'
exit_timestamp = pd.to_datetime(datetime_str)
...
# %% COMBINING CE AND PE strike Price
...
# %% straddle dataframe by combinning these two
straddle_df = pd.merge(rel_CE_df,rel_PE_df, left_index=True, right_index=True)
...
straddle_entryPriceValue = straddle_df.loc[straddle_df.index.time == entry_time_dt ]['straddle_close'].values[0]


straddle_df['straddle_entryPrice'] = np.NaN
straddle_df.loc[straddle_df.index.time > entry_time_dt, 'straddle_entryPrice'] = straddle_entryPriceValue

#Creating straddle_stoploss
straddle_df['straddle_stoploss'] =np.NaN
straddle_df.loc[straddle_df.index.time > entry_time_dt, 'straddle_stoploss'] = straddle_entryPriceValue*(1+straddle_SL_pct)

#Creating target
straddle_df['straddle_tgt'] = np.NaN
straddle_df.loc[straddle_df.index.time > entry_time_dt, 'straddle_tgt'] = straddle_entryPriceValue*(1 - straddle_tgt_pct)
...
condition3 = straddle_df.index.time == exit_time_dt
...
straddle_df.loc[((straddle_df.index > index_position) & (straddle_df.index < exit_timestamp)), 'straddle_hit_status'] = first_non_null
...
condition = (straddle_df.index.time >= entry_time_dt) & (straddle_df.index.time <= end_time_dt)
...
# straddle_df.to_csv('straddle_df2.csv')