...
class Straddle:
    ...
    def __init__(self, date, underlying, exp_period,
       ...
       #initializing dataframes 
        ...
        self.read_bank_nifty_data()
        self.extract_unique_dates()
        self.read_weekly_exp_dates()
        self.process_exp_dates()
    ...
    def read_bank_nifty_data(self):
        try:
            filepath = f'C:\\keshav\\Rahul\\Options_data\\NIFTY & BANKNIFTY (Jan 2020 to 19 Dec 2022)\\NIFTY & BANKNIFTY Spot Indices (Jan 2020 to 19 Dec 2022)\\{self.underlying}.csv'

            bank_nifty_df = pd.read_csv(filepath, header=None)
            ...
            self.bank_nifty_df = bank_nifty_df
    ...
    def extract_unique_dates(self):
        x = sorted(list(set(self.bank_nifty_df.index.date)))
        x = pd.to_datetime(x)
        self.unique_date_list = [date.strftime("%Y-%m-%d") for date in x]
        # print(self.unique_date_list)



    def read_weekly_exp_dates(self):
        try:
            filepath = "C:\keshav\Rahul\Options_data\Weekly_exp_dates.csv"
            weekly_exp_df = pd.read_csv(filepath, usecols=['Exp_date'])
            ...
            self.weekly_exp_df = weekly_exp_df
            # print(weekly_exp_df)
    ...
    def process_exp_dates(self):
        for i in range(len(self.weekly_exp_df)):
            try:
                if self.weekly_exp_df.loc[i, 'day'] > self.weekly_exp_df.loc[i+1, 'day']:
                    self.weekly_exp_df.loc[i, 'monthly_exp'] = True
                monthly_exp_df = pd.DataFrame()
                monthly_exp_df = self.weekly_exp_df.loc[(self.weekly_exp_df['monthly_exp'] == True), ['Exp_date', 'day']]
                self.df_weekly_list = self.weekly_exp_df[['Exp_date']]
                self.df_monthly_list = monthly_exp_df[['Exp_date']]

                #making newe dataframe
                self.df_datelist = pd.DataFrame(self.bank_nifty_df['date'].unique(), columns = ['date'])
                self.df_datelist[['currWeek_exp_dt', 'nextWeek_exp_dt', 'farWeek_exp_dt','currMonth_exp_dt', 'nextMonth_exp_dt', 'farMonth_exp_dt' ]] = np.NaN

            except: 
                pass


        for i in range(len(self.df_datelist)):
            try:
                df2 = self.df_weekly_list.loc[self.df_weekly_list['Exp_date'] >= self.df_datelist['date'][i]]
                self.df_datelist.loc[i,'currWeek_exp_dt'] = df2['Exp_date'].values[0]
                self.df_datelist.loc[i,'nextWeek_exp_dt'] = df2['Exp_date'].values[1]
                self.df_datelist.loc[i,'farWeek_exp_dt'] = df2['Exp_date'].values[2]
                df3 = self.df_monthly_list.loc[self.df_monthly_list['Exp_date'] >= self.df_datelist['date'][i]]

                self.df_datelist.loc[i,'currMonth_exp_dt'] = df3['Exp_date'].values[0]
                self.df_datelist.loc[i,'nextMonth_exp_dt'] = df3['Exp_date'].values[1]
                self.df_datelist.loc[i,'farMonth_exp_dt'] = df3['Exp_date'].values[2]
            
            except:
                pass    


        merged_df = pd.merge(self.bank_nifty_df,self.df_datelist,how='left', on='date')
        ...
        self.bank_nifty_df.index = pd.to_datetime(self.bank_nifty_df.index)
...
straddle_instance = Straddle(date='2022-03-10', underlying='BANKNIFTY', exp_period='currWeek_exp_dt',
