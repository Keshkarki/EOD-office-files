...
class MergedDataFrame:
    ...
    def get_unique_dates(self):
            unique_dates = sorted(list(set(self.get_merged_df().index.date)))
            unique_dates = pd.to_datetime(unique_dates).strftime("%Y-%m-%d").to_list()
            return unique_dates

    

    def guide(self):
        print('1. merged_df = merged_df_obj.get_merged_df()')
        print("2. alywas run this command after object creation so it can store value of merged df and we will use it further")
        print("3. also same underying should be pass in antoher classes using this class")
        print("4. Choose Underlying ['BANKNIFY','NIFTY']")
