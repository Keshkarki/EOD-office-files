...
Intra_merged_df = pd.merge(df,df_datelist,how='left', on='date')
Intra_merged_df['currWeek_exp_dt'] = pd.to_datetime(Intra_merged_df['currWeek_exp_dt']).dt.date
Intra_merged_df['nextWeek_exp_dt'] = pd.to_datetime(Intra_merged_df['nextWeek_exp_dt']).dt.date
Intra_merged_df['farWeek_exp_dt'] = pd.to_datetime(Intra_merged_df['farWeek_exp_dt']).dt.date
Intra_merged_df['currMonth_exp_dt'] = pd.to_datetime(Intra_merged_df['currMonth_exp_dt']).dt.date
Intra_merged_df['nextMonth_exp_dt'] = pd.to_datetime(Intra_merged_df['nextMonth_exp_dt']).dt.date
Intra_merged_df['farMonth_exp_dt'] = pd.to_datetime(Intra_merged_df['farMonth_exp_dt']).dt.date

Intra_merged_df['date'] = Intra_merged_df['date'].astype(str)
Intra_merged_df['time'] = Intra_merged_df['time'].astype(str)
datetime = pd.to_datetime(Intra_merged_df['date'] + ' ' + Intra_merged_df['time'], format='mixed', dayfirst=True)
df.index = pd.to_datetime(df.index)
Intra_merged_df.insert(0,'datetime',datetime) 
Intra_merged_df.set_index('datetime',inplace=True)
Intra_merged_df.drop(columns=['date','time'],inplace=True)
Intra_merged_df = Intra_merged_df.loc[Intra_merged_df.index.date == date_dt]
...
for date_dt in unique_lst:
    try:
        date = date_dt.strftime('%Y-%m-%d')   #string --> 2020-01-01
        datestr = date.split('-')[0]           #--> 2020

        Intra_Intra_merged_df = Intra_merged_df.loc[Intra_merged_df.index.date == date_dt]
        ...
        Intra_merged_df.to_csv('Hello3.csv')
        ...
        #merging dataframes Intra_merged_df,rel_CE_df and rel_PE_df and make final_df
