...
date_list  = obj.get_unique_dates()

# %%

class Individual:
    def __init__(self,merged_df, dates, exp_period, underlying,
    ...
    def process_dates(self):
        self.final_result_df_all = pd.DataFrame()
        ...
        for date in self.dates:
            try:
                ...
                # Perform processing for each date
                # Use self.merged_df, self.exp_period, self.underlying, etc., for computations
                print("Processing date:", self.date)

                #conversion
                self.date_dt = pd.to_datetime(self.date).date()
                ...
                self.datetime_str = f'{date} {self.exit_time}'
                self.exit_timestamp = pd.to_datetime(self.datetime_str)
                # print(self.exit_timestamp)
        ...
        # FOR PE
                ...
                self.final_result_df_all = pd.concat([self.final_result_df_all,self.result_df],ignore_index=True)


            except Exception as e:
                print(f"Some Error Occured For {date}, Error: {e}")
        
# Create an instance of the individual class with custom values
obj = Individual(merged_df = merged_df,dates=date_list,
