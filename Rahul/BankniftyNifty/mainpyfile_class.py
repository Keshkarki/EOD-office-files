    ...
    for i, row in df.iterrows():
        ...
        try:
            ...
            if dt in option_df2.index:
                ...
                df.loc[dt, f'{option_type}price'] = price_value
            else:
                df.loc[dt, f'{option_type}price'] = np.NaN
...
def process_option_data_exit(symbol,option_type):
    for i, row in df.iterrows():
        ...
        try:
            ...
            if dt in option_df2.index:
                price_value = option_df2.loc[dt, f'{option_type}close']
                df.loc[df['exitDt']==dt, f'Exit_{option_type}price'] = price_value
            else:
                df.loc[df['exitDt']==dt, f'Exit_{option_type}price'] = np.NaN
        except FileNotFoundError:
            pass
