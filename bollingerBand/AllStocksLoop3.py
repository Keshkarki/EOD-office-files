...
# %% 
# list_of_companies = ['HDFCBANK.NS.csv']
...
for i in range(len(list_of_companies)):
        ...
        # target ratio changed here
        mergedDf['targetRatio'] = np.nan


        for i in range(mergedDf.shape[0]):
                mergedDf['targetRatio'] = np.where(
                                                pd.notna(mergedDf['entrySignal']),
                                                mergedDf['mean'],
                                                mergedDf['targetRatio'].shift()
                                                )

                mergedDf['targetRatio'] = mergedDf['targetRatio']
        ...
        ############### PAIRED PNL
        # niftyEntryPrice
        mergedDf['niftyEntryPrice'] = np.nan
        for i in range(mergedDf.shape[0]):
                mergedDf['niftyEntryPrice'] = np.where(
                                                pd.notna(mergedDf['entrySignal']),
                                                mergedDf['niftyClose'],
                                                mergedDf['niftyEntryPrice'].shift()
                                                        )


        #niftyExitPrice
        mergedDf['niftyExitPrice'] = np.where(
                            pd.notna(mergedDf['exitPrice']),mergedDf['niftyClose'],np.NaN
)



        #niftyPnL
        mergedDf['niftyPnL'] = np.where(pd.notna(mergedDf['niftyExitPrice']),
                np.where(mergedDf['signal'] == 'Buy', mergedDf['niftyEntryPrice'] - mergedDf['niftyExitPrice'], mergedDf['niftyExitPrice'] - mergedDf['niftyEntryPrice']
        ...
        #niftyPnlPercent
        mergedDf['niftyPnlPercent'] = (mergedDf['niftyPnL'] / mergedDf['niftyEntryPrice'])*100


        #combinePnLpercent
        mergedDf["combinePnLPercent"] = mergedDf['PnLPercent']/2 + mergedDf['niftyPnlPercent']/2
        ...
        #Last Step filtering ***************************************
        filterdDf = mergedDf[(mergedDf['combinePnLPercent']!=0) & (pd.notna(mergedDf['combinePnLPercent']))]['combinePnLPercent']
...
# %%
final_result = pd.pivot_table(final_result, values='combinePnLPercent', index='date', columns='companyName')
final_result
